...
void Bound::actByHooke(double dtime) {
    ...
    double first_damping_x =  -damping * (atomPair_.first->get_vel().x_ - atomPair_.second->get_vel().x_);
    double first_damping_y =  -damping * (atomPair_.first->get_vel().y_ - atomPair_.second->get_vel().y_);
    Point2D acc_first(force_x_first + first_damping_x, force_y_first + first_damping_y);
    Point2D acc_second(-force_x_first + -first_damping_x,-force_y_first + -first_damping_y);
...
void AtomChain::actByGravity(double time, double g) {

    for(auto it = atoms_.begin();it<atoms_.end();it++){
        it->accelerate(Point2D{0,g},time);
    }
}

int AtomChain::getNearest(int x, int y) {
    double distance;
    double best_distance = INFINITY;
    int r_index;
    for(int i = 0; i < (atoms_.size()); i++){
        auto pos = atoms_[i].get_pos();
        distance = (pos.x_ - x)*(pos.x_ - x) + (pos.y_ - y)*(pos.y_ - y);
